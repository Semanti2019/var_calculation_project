...
class TestVaRCalculation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.valid_config: List[Tuple[pd.DataFrame, float, float, callable]] = [
            (self.df[['ccy_1']], self.horizon_days, self.portfolio_value, lambda time0_value, time1_value, horizon_days: 9)
        ]
    ...
    def test_empty_dataframe(self):
        config = [
            (pd.DataFrame(), self.horizon_days, self.portfolio_value, lambda time0_value, time1_value, horizon_days: 0.01)
        ]
